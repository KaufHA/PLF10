...
bool ESPPreferenceObject::load_() {
  ...
  ESP_LOGV(TAG, "LOAD %u: valid=%s, 0=0x%08X 1=0x%08X (Type=%u, CRC=0x%08X)", this->offset_,  // NOLINT
...
bool ESPPreferenceObject::save_() {
  ...
  if (!this->save_internal_())
    return false;
  ESP_LOGV(TAG, "SAVE %u: 0=0x%08X 1=0x%08X (Type=%u, CRC=0x%08X)", this->offset_,  // NOLINT
...
bool ESPPreferenceObject::load_internal_() {
  if (this->in_flash_) {
    for (uint32_t i = 0; i <= this->length_words_; i++) {
      ...
      ESP_LOGV(TAG, "%u=%08X",i,this->data_[i]);
    }
...
ESPPreferenceObject ESPPreferences::make_preference(size_t length, uint32_t type, bool in_flash) {
  if (in_flash) {
    ...
    // should never get to this because ESPHome only saves a pretty small number of well defined things.
      ...
      if ( this->current_flash_offset_ < 40 ) {this->current_flash_offset_ = 40;}
      start = this->current_flash_offset_;
    }

    ESP_LOGV(TAG, "Making Preference in Flash - start: %u: length: %u, type: %u", start, length, type);
